...
from model_softmax import MuRIL_Softmax  # Use Softmax model instead of CRF
...
BASE_SAVE_DIR = "weights/no_crf"  # Changed from SAVE_DIR to BASE_SAVE_DIR
...
def validate(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Get predictions (Softmax returns list of lists directly)
...
def train(seed: int, save_dir: str):
    """Main training function with SOFTMAX (No CRF)."""
    ...
    logger.info("="*60)
    logger.info(f"NO CRF EXPERIMENT - SEED: {seed}")
    logger.info("="*60)
    ...
    # Initialize model (Softmax instead of CRF)
    logger.info("Using Softmax classification (No CRF)")
...
if __name__ == "__main__":
    ...
    print("\nStarting No-CRF statistical robustness experiment with 3 seeds...")
    print("="*60)
